...
contract GCAT is ERC20 {
    ...
    function createPool(uint256 tokenA, uint256 tokenB) external onlyOwner {
    require(tokenA > 0, "Token A amount must be greater than 0");
    require(tokenB > 0, "Token B amount must be greater than 0");
    require(balanceOf(address(this)) >= tokenA, "Insufficient Token A balance");

    _approve(address(this), address(uniswapRouter), tokenA);
